...
For the `main` branch it is necessary to implement `TryFrom<JsonObject>` and `Into<JsonObject>` for your type.
For derive macro usage please refer to [readme file](https://github.com/Fikoslavv/rusty-json/blob/dev-macros-derive/README.md) at branch `dev-macros-derive`.
...
fn demo_deserialization<T>(str: String) where T : TryFrom<JsonObject>, for <'a> &'a T : TryFrom<JsonObject>, <T as TryFrom<rusty_json::JsonObject>>::Error: std::fmt::Debug
{
